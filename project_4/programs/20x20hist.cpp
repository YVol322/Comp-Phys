...
int main()
{
    ...
    int n = 10000 * N; //number of MCMC cyclce
    ...
    std::string filename = "20x20_table_hist.csv";
    std::ofstream ofile;
    ofile.open("20x20_table_hist.csv"); 
    //std::string filename = "20x20_T=2.4_table.csv";
    //std::ofstream ofile;
    //ofile.open("20x20_T=2.4_table.csv"); 

    ofile <<"n, eps [J]" << std::endl;
...
//
        ...
        for(int k = 1; k<=n;k++)
        {
            ...
            << std::setw(width) << std::setprecision(prec) << std::scientific << E << std::endl; 
        }
    }
