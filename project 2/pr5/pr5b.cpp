...
void JacF(arma::mat& A, arma::mat& R, int k, int l, int N, double e, int& j); //aded int j - number of iterations

int main()
{
    int N;

    std::string filename = "pr5btable.csv";
    std::ofstream ofile;
    ofile.open("pr5btable.csv"); 
    ...
    for(N=4; N<15; N++)
    {
        ...
        arma::mat A = arma::mat(N, N).randn();//dence A matrix

        A = arma::symmatu(A); //making A symetric
        ...
        //if you want to make sure algo is working well for N != 6 uncomment next lines 79-80.
