...
file = open("/Users/JVol/Documents/GitHub/comp-phys/project3/multiple_particles/FE4_table_100ms.csv")
type(file)
...
for i in range(len(rows)):
    x.append(float(rows[i][6].strip()))
    y.append(float(rows[i][7].strip()))
    z.append(float(rows[i][8].strip()))
    vx.append(float(rows[i][9].strip()))
    vy.append(float(rows[i][10].strip()))
    vz.append(float(rows[i][11].strip()))
